...
struct IRuntimeOnlyCostEstimator {
  virtual RuntimeOnlyOpCostMetrics estimate_cost(RuntimeOnlyOpCostEstimateKey const &) const = 0;
  ...
  IRuntimeOnlyCostEstimator &operator=(IRuntimeOnlyCostEstimator const &) = delete;
...
struct RuntimeOnlyCostEstimator {
  RuntimeOnlyOpCostMetrics estimate_cost(RuntimeOnlyOpCostEstimateKey const &) const;
  milliseconds_t estimate_cost(TensorSetMovement const &m) const;

  template <typename T, typename... Args>
  static typename std::enable_if<std::is_base_of<IRuntimeOnlyCostEstimator, T>::value,
                                 RuntimeOnlyCostEstimator>::type
      create(Args &&...args) {
    return RuntimeOnlyCostEstimator(std::make_shared<T>(std::forward<Args>(args)...));
  }

private:
  RuntimeOnlyCostEstimator(std::shared_ptr<IRuntimeOnlyCostEstimator> implementation_ptr);
