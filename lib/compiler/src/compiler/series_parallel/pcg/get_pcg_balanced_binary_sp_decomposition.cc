...
std::optional<PCGBinarySPDecomposition>
  ...
  std::optional<SeriesParallelDecomposition> spd = get_pcg_series_parallel_decomposition(pcg);
  if (!spd) {
    return std::nullopt;
  }
  return pcg_binary_sp_decomposition_from_binary_sp_tree(balanced_binary_sp_tree_from_nary(spd.value()));
}
