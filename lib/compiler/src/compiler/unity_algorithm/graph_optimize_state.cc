...
GraphOptimizeState::GraphOptimizeState(
    ParallelComputationGraph const &pcg, milliseconds_t runtime)
...
static std::unordered_multiset<std::tuple<
  ...
  auto get_layer_signature = [&](parallel_layer_guid_t l)
    ...
    ParallelLayerAttrs layer_attrs =
        get_parallel_layer_attrs(pcg, l);

    std::unordered_map<
        TensorSlotName,
        std::tuple<ParallelLayerAttrs, TensorSlotName, ParallelTensorAttrs>>
        inputs =
            map_values(get_incoming_tensors(pcg, l),
...
bool GraphOptimizeState::operator==(GraphOptimizeState const &other) const {
  return get_layer_signature_set(this->pcg) == get_layer_signature_set(other.pcg);
}
...
//   graph_optimize_state_from_machine_mapping_result(ParallelComputationGraph const &pcg, 
//                                                    PCGBinarySPDecomposition const &binary_sp_decomposition,
...
//             assert_unwrap(compgraph_op_attrs_from_pcg_op_attrs(pcg_get_op_attrs(pcg, layer_guid)));
//
