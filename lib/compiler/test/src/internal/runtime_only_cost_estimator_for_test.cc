  ...
  return runtime_only_cost_estimator_from_cost_estimator(
    make_fake_cost_estimator(
      [get_operator_cost](OpCostEstimateKey const &key) -> OpCostMetrics { 
        RuntimeOnlyOpCostMetrics runtime_only_op_cost_metrics = get_operator_cost(
          runtime_only_from_op_cost_estimate_key(key)); 
        return make_op_cost_metrics_from_runtime_only(runtime_only_op_cost_metrics, 0_bytes);
      },
      get_communication_cost));
}
...
RuntimeOnlyCostEstimator make_fake_runtime_only_cost_estimator(
  ...
  return make_fake_runtime_only_cost_estimator(
      [op_cost_map](RuntimeOnlyOpCostEstimateKey const &k) { return op_cost_map.at(k); },
