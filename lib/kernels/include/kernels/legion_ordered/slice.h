...
template <typename T>
LegionOrdered<T> slice(LegionOrdered<T> const &d, legion_dim_t const &start, std::optional<legion_dim_t> const &end) {
  int raw_start = start.value.unwrap_nonnegative();
  std::optional<int> raw_end = transform(end, [](legion_dim_t const &i) { return i.value.unwrap_nonnegative(); });
