...
void forward_kernel(ffStream_t stream,
  ...
  coord_t stride =
      output.shape.sub_shape(std::nullopt, add_to_legion_dim(m.legion_dim, 1))
          .num_elements().unwrap_nonnegative();
...
void backward_kernel(ffStream_t stream,
  ...
  coord_t stride =
      output_grad.shape
          ...
          .get_volume().unwrap_nonnegative();
  coord_t output_dim_size = output_grad.shape.at(m.legion_dim).unwrap_nonnegative();
  coord_t input_dim_size = input_grad.shape.at(m.legion_dim).unwrap_nonnegative();
