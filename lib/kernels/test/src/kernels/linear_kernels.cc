...
TEST_SUITE(FF_CUDA_TEST_SUITE) {
  TEST_CASE("forward_kernel CPU/GPU alignment (Linear)") {
    ...
    auto run_forward_kernel = [&](DeviceType device_type) {
      ...
      std::optional<ManagedPerDeviceFFHandle> managed_handle = 
        create_local_handle_for_device_type(device_type, 
    ...
    CHECK_MESSAGE(accessors_are_equal(cpu_result, gpu_result),
  ...
  TEST_CASE("backward_kernel CPU/GPU alignment (Linear)") {
    ...
    auto run_forward_kernel = [&](DeviceType device_type) {
      ...
      GenericTensorAccessorW input_grad = allocator.allocate_tensor(
      ...
      GenericTensorAccessorW bias_grad = allocator.allocate_tensor(
      ...
      TensorShape output_shape = TensorShape{
        TensorDims{FFOrdered{positive_int{batch_size}, positive_int{output_num_channels}}},
        DataType::FLOAT,
      };
    ...
    CHECK_MESSAGE(accessors_are_equal(cpu_input_grad, gpu_input_grad),
    ...
    CHECK_MESSAGE(accessors_are_equal(cpu_projection_grad, gpu_projection_grad),
                  check_kv("cpu_projection_grad", format_accessor_w_contents(cpu_projection_grad)),
                  check_kv("gpu_projection_grad", format_accessor_w_contents(gpu_projection_grad)));

    CHECK_MESSAGE(accessors_are_equal(cpu_bias_grad, gpu_bias_grad),
