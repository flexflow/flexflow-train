...
TEST_SUITE(FF_TEST_SUITE) {
  ...
  TEST_CASE("tensor_accessor_transpose") {
    ...
    GenericTensorAccessorW result = tensor_accessor_transpose(input, cpu_allocator);
  ...
  TEST_CASE("tensor_accessor_reduce") {
    ...
    SUBCASE("inner dim") {
      GenericTensorAccessorW result = tensor_accessor_reduce(input, ff_dim_t{1_n}, cpu_allocator);

      GenericTensorAccessorR correct = create_1d_accessor_r_with_contents<float>(
            {
                3 + 3 + 6,
                0 + (-1) + 0.75,
            },
    ...
    SUBCASE("outer_dim") {
      GenericTensorAccessorW result = tensor_accessor_reduce(input, ff_dim_t{0_n}, cpu_allocator);
