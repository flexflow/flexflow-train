...
LocalTensorBacking construct_local_tensor_backing(
  ...
  std::unordered_set<training_tensor_guid_t> to_allocate =
    set_minus(keys(training_tensor_shapes),
              keys(preallocated));

  std::unordered_map<training_tensor_guid_t, GenericTensorAccessorW> 
    allocated = generate_map(to_allocate,
  ...
  std::unordered_map<training_tensor_guid_t, GenericTensorAccessorW> 
    backing_for_training_tensor_map = merge_disjoint_maps(allocated, preallocated);
...
GenericTensorAccessorW
    get_accessor_for_training_tensor(LocalTensorBacking const &local_tensor_backing, training_tensor_guid_t training_tensor) {
  return local_tensor_backing.backing_for_training_tensor_map.at(training_tensor);
}
...
std::unordered_map<tensor_sub_slot_id_t, TensorSlotBacking>
  ...
  return map_values(binding.get_tensor_bindings(),
                    [&](training_tensor_guid_t t) {
