...
LocalTrainingBacking make_local_training_backing_for_computation_graph(
     Allocator &allocator,
     std::unordered_map<training_tensor_guid_t, GenericTensorAccessorW> const &preallocated,
  ...
  LocalTaskRegistry local_task_registry = 
    construct_local_task_registry_for_layers(
      get_layer_attrs_mapping(training_computation_graph.computation_graph));

  LocalTensorBacking local_tensor_backing =
      construct_local_tensor_backing(
...
std::optional<milliseconds_t>
  ...
  ASSERT(registry_contains_task_for_layer(local_task_registry, training_layer.layer_guid, OpTaskType::FWD));

  std::optional<DeviceSpecificDeviceStates> device_state =
      get_per_device_op_state_if_exists(
          local_args_backing, training_layer.layer_guid);

  TaskInvocation invocation = lower_to_task_invocation(
      /*op_task_invocation=*/get_forward_op_task_invocation(training_layer.layer_attrs.op_attrs),
  ...
  return call_task_impl(
      local_task_registry, invocation.task_id, accessor);
}
...
void compute_loss(LocalTrainingBacking const &local_training_backing,
  ...
  TaskInvocation loss_invocation = backward(
      loss_attrs,
      get_forward_tensor_guid_for_tensor_guid(local_training_backing.training_computation_graph, logit_tensor),
      get_gradient_tensor_guid_for_tensor_guid(local_training_backing.training_computation_graph, logit_tensor),
      label_tensor);
...
std::optional<milliseconds_t>
    execute_backward(LocalTaskRegistry const &local_task_registry,
                     ...
                     TrainingLayerPlusContext const &training_layer,
                     Allocator &allocator) {
  if (!registry_contains_task_for_layer(local_task_registry,
...
void execute_update(LocalTrainingBacking const &local_training_backing,
  ...
  if (training_layer.layer_attrs.op_attrs.has<WeightAttrs>()) {
    TrainingTensorGroupWithAttrs weight_tensor_group = get_only(training_layer.output_tensor_groups);
