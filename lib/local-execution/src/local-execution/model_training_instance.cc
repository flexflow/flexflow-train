...
std::unordered_map<layer_guid_t, std::optional<milliseconds_t>> ModelTrainingInstance::forward() {
  std::unordered_map<layer_guid_t, std::optional<milliseconds_t>> per_layer_elapsed_time;
  for (layer_guid_t const &layer_guid :
       topological_ordering(this->training_backing.training_computation_graph.computation_graph)) {
    std::optional<milliseconds_t> elapsed_time =
        execute_forward(
          ...
          get_training_layer_plus_context(this->training_backing.training_computation_graph, layer_guid),
          this->allocator);
...
std::unordered_map<layer_guid_t, std::optional<milliseconds_t>> ModelTrainingInstance::backward() {
  ...
  std::unordered_map<layer_guid_t, std::optional<milliseconds_t>> per_layer_elapsed_time;
  for (layer_guid_t const &layer_guid : reversed(
           topological_ordering(this->training_backing.training_computation_graph.computation_graph))) {
    std::optional<milliseconds_t> elapsed_time =
        execute_backward(
          this->training_backing.local_task_registry, 
...
void ModelTrainingInstance::update() {
  for (layer_guid_t const &layer_guid :
       topological_ordering(this->training_backing.training_computation_graph.computation_graph)) {
    execute_update(
        this->training_backing, layer_guid, this->optimizer_attrs, this->allocator);
  }
...
GenericTensorAccessorR ModelTrainingInstance::get_loss_tensor_accessor() const {
  gradient_tensor_guid_t loss_tensor =
  ...
  GenericTensorAccessorW loss_tensor_backing =
      this->training_backing.local_tensor_backing.backing_for_training_tensor_map.at(
