...
static DeviceSpecificDeviceStates
  ...
  nonnegative_int reduction_size = input.shape.get_volume() / output.shape.get_volume();
  ReducePerDeviceState per_device_state =
      init_kernel(handle, op_type, reduction_size.unwrap_nonnegative(), input.shape, output.shape);
