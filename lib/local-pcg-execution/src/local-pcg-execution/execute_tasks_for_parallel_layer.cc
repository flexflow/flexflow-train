      ...
      [&](AtomicTaskInvocation const &atomic_task_invocation)
        ...
        TaskArgumentAccessor task_arg_accessor =
            get_task_arg_accessor_for_atomic_task_invocation(
                atomic_tensor_backing, atomic_task_invocation, allocator);
...
std::optional<MappedPerDeviceOpStatesGroup> execute_init_for_parallel_layer(
  ...
  std::unordered_map<MachineSpaceCoordinate, LocalReadyToLaunchTask>
      prepared_tasks =
          prepare_parallel_runtime_task_invocations(runtime_task_invocation,
                                                    parallel_tensor_backing,
  ...
  std::unordered_map<MachineSpaceCoordinate,
      ...
      op_state_by_shard = map_values(
          prepared_tasks,
          [&](LocalReadyToLaunchTask const &prepared_task)
              -> std::optional<DeviceSpecificPerDeviceOpState> {
            return call_init_task_impl(task_registry,
  ...
  return transform(
      ...
      [](std::unordered_map<MachineSpaceCoordinate,
                            DeviceSpecificPerDeviceOpState> const &m) {
...
static std::optional<TaskGroupExecutionTimes> execute_fwb_for_parallel_layer(
  ...
  OpTaskType op_task_type =
      assert_unwrap(op_task_type_from_fwb_op_task_type(fwb_task_type));
  ...
  std::unordered_map<MachineSpaceCoordinate, std::optional<milliseconds_t>>
      timing_by_shard = map_values(
          prepared_tasks,
          [&](LocalReadyToLaunchTask const &prepared_task)
              -> std::optional<milliseconds_t> {
            return call_fwb_task_impl(task_registry,
                                      prepared_task.task_id,
                                      prepared_task.task_arg_accessor);
          });
...
std::optional<TaskGroupExecutionTimes> execute_forward_for_parallel_layer(
  ...
  return execute_fwb_for_parallel_layer(symbolic_layer_guid,
                                        g,
                                        parallel_tensor_backing,
                                        atomic_tensor_backing,
                                        allocator,
