...
MappedRuntimeTaskGroup::MappedRuntimeTaskGroup(
  ...
  auto check_arity = [&](TensorRole tensor_role) -> nonnegative_int {
    std::unordered_set<nonnegative_int> arities = transform(
        shard_bindings.right_values(),
  ...
  for (TaskSignatureTensorKey const &key : all_keys) {
    std::vector<RuntimeAtomicTaskShardBinding> signatures_for_key =
...
bool MappedRuntimeTaskGroup::operator==(
    MappedRuntimeTaskGroup const &other) const {
...
bidict<MachineSpaceCoordinate, RuntimeAtomicTaskShardBinding> const &
    MappedRuntimeTaskGroup::get_shard_bindings() const {
  return this->shard_bindings;
}

std::string format_as(::FlexFlow::MappedRuntimeTaskGroup const &m) {
  return fmt::format("<MappedRuntimeTaskGroup shard_bindings={}>",
                     m.get_shard_bindings());
}

std::ostream &operator<<(std::ostream &s,
                         ::FlexFlow::MappedRuntimeTaskGroup const &x) {
      ...
      transform_values(
          ...
          [&](RuntimeAtomicTaskShardBinding const &op_shard_binding)
            ...
            return lower_op_shard_binding_to_runtime_shard_binding(
                op_shard_binding, symbolic_layer_signature, task_type);
          }),
  };
}
...
size_t hash<::FlexFlow::MappedRuntimeTaskGroup>::operator()(
    ::FlexFlow::MappedRuntimeTaskGroup const &x) const {
