...
static std::unordered_map<symbolic_training_tensor_guid_t,
  ...
  std::vector<symbolic_training_tensor_guid_t> keys =
      get_training_tensors_for_role_and_type(
          signature, tensor_role, tensor_type);
...
RuntimeAtomicTaskShardBinding lower_op_shard_binding_to_runtime_shard_binding(
  ...
  auto get_bindings = [&](TensorRole tensor_role, FwbTensorType tensor_type) {
    return get_tensor_shard_binding_for_type(
        signature, op_shard_binding, tensor_role, tensor_type);
  };
...
RuntimeAtomicTaskShardBinding lower_op_shard_binding_to_runtime_shard_binding(
  ...
  switch (task_type) {
    case FwbOpTaskType::FWD:
      return lower_op_shard_binding_to_fwd_pass_runtime_shard_binding(
          shard_binding, signature);
    case FwbOpTaskType::BWD:
      return lower_op_shard_binding_to_bwd_pass_runtime_shard_binding(
          shard_binding, signature);
    default:
