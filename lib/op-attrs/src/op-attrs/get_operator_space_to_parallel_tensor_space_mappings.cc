...
std::unordered_map<TensorSlotName, OperatorSpaceToParallelTensorSpaceMapping> 
  ...
  >(overload {
      [&](ElementBinaryAttrs const &attrs)
        ...
        ParallelTensorDimDegrees lhs_degrees = inputs_degrees.at(TensorSlotName::LHS_INPUT);
        ParallelTensorDimDegrees rhs_degrees = inputs_degrees.at(TensorSlotName::RHS_INPUT);
      ...
      [&](ElementUnaryAttrs const &attrs) 
        -> std::unordered_map<TensorSlotName, OperatorSpaceToParallelTensorSpaceMapping> {
        ParallelTensorDimDegrees input_degrees = require_only_key(inputs_degrees, TensorSlotName::INPUT);
      ...
      [&](InputAttrs const &) { 
        ...
        return std::unordered_map<TensorSlotName, OperatorSpaceToParallelTensorSpaceMapping>{};
      },
      [&](LinearAttrs const &attrs) 
        -> std::unordered_map<TensorSlotName, OperatorSpaceToParallelTensorSpaceMapping> {
        ParallelTensorDimDegrees input_degrees = require_only_key(inputs_degrees, TensorSlotName::INPUT);

        std::unordered_map<TensorSlotName, OperatorSpaceToParallelTensorSpaceMapping> result = {
          {TensorSlotName::INPUT, get_operator_to_input_mapping(attrs, input_degrees)},
          {TensorSlotName::WEIGHT, get_operator_to_projection_mapping(attrs, input_degrees)},
        };

        if (attrs.use_bias) {
          result.insert({TensorSlotName::BIAS, get_operator_to_bias_mapping(attrs, input_degrees)});
        };
      ...
      [](auto const &attrs) -> std::unordered_map<TensorSlotName, OperatorSpaceToParallelTensorSpaceMapping> {
...
std::unordered_map<TensorSlotName, OperatorSpaceToParallelTensorSpaceMapping>
  ...
  >(overload {
      [&](ElementBinaryAttrs const &attrs)
        -> std::unordered_map<TensorSlotName, OperatorSpaceToParallelTensorSpaceMapping>
      {
        auto [lhs_degrees, rhs_degrees] = require_two_keys(inputs_degrees, TensorSlotName::LHS_INPUT, TensorSlotName::RHS_INPUT);
