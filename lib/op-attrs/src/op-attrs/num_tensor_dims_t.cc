...
num_tensor_dims_t::num_tensor_dims_t(nonnegative_int value_) : value(value_) {
...
nonnegative_int
    num_tensor_dims_t::nonnegative_int_from_num_tensor_dims() const {
  return this->value;
}
...
num_tensor_dims_t num_tensor_dims_from_num_ptensor_shard_dims(
    num_ptensor_shard_dims_t num_ptensor_shard_dims) {
  return num_tensor_dims_t{num_ptensor_shard_dims.value};
}

num_tensor_dims_t num_tensor_dims_from_num_ptensor_parallel_dims(
    num_ptensor_parallel_dims_t num_ptensor_parallel_dims) {
  return num_tensor_dims_from_num_ptensor_shard_dims(
      num_ptensor_shard_dims_from_parallel_dims(num_ptensor_parallel_dims));
}

num_ptensor_shard_dims_t num_ptensor_shard_dims_from_num_tensor_dims(
    num_tensor_dims_t num_tensor_dims) {
...
num_ptensor_parallel_dims_t num_ptensor_parallel_dims_from_num_tensor_dims(
  ...
  return num_ptensor_parallel_dims_from_shard_dims(
      num_ptensor_shard_dims_from_num_tensor_dims(num_tensor_dims));
}
...
std::vector<relative_ff_dim_t>
    relative_tensor_dims_range(num_tensor_dims_t num_tensor_dims) {
  return transform(
      nonnegative_range(num_tensor_dims.nonnegative_int_from_num_tensor_dims()),
      [](nonnegative_int idx) {
