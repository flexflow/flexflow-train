...
std::unordered_map<parallel_tensor_dim_idx_t, positive_int>
  ...
  std::unordered_map<parallel_tensor_dim_idx_t, positive_int> replica_dim_degrees = {
...
std::unordered_set<ParallelTensorSpaceCoordinate> 
  ...
  std::unordered_map<parallel_tensor_dim_idx_t, positive_int> degree_map = get_parallel_tensor_degree_map(degrees);

  std::unordered_map<
    parallel_tensor_dim_idx_t, 
    std::unordered_set<nonnegative_int>> possible_per_dim_coords
    = map_values(degree_map, [](positive_int degree) { return unordered_set_of(nonnegative_range(degree)); });
