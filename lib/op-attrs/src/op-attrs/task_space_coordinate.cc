...
TaskSpaceCoordinate 
  ...
  std::unordered_set<operator_task_space_dim_idx_t> coord_dims = get_coord_dims(dim_coord);

  std::set<operator_task_space_dim_idx_t> dims = operator_task_space_dim_idx_range(num_elements(coord_dims));

  ASSERT(coord_dims == unordered_set_of(dims));

  std::unordered_map<nonnegative_int, nonnegative_int> idx_map = map_keys(
    dim_coord.raw, [](operator_task_space_dim_idx_t idx) {
      return idx.raw_idx;
    });
...
DimCoord<operator_task_space_dim_idx_t>
  ...
  return dim_coord_from_orthotope_coord(
    ...
    unordered_set_of(operator_task_space_dim_idx_range(orthotope_coord_num_dims(coord.orthotope_coord))),
