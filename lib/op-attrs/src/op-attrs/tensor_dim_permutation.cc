...
std::tuple<bidict<ff_dim_t, ff_dim_t> const &> TensorDimPermutation::tie() const {
...
TensorDimPermutation compose_tensor_dim_permutations(
  TensorDimPermutation const &lhs,
...
static FFOrdered<T> permute_ff_ordered(TensorDimPermutation const &permutation,
  ...
  return ff_ordered_from_map(
      map_keys(
        map_from_ff_ordered(ff_ordered),
        [&](ff_dim_t k) {
          return permutation.at_l(k);
        }));
}


TensorDims
  permute_tensor_dims(TensorDimPermutation const &permutation,
...
TensorShape
  permute_tensor_shape(TensorDimPermutation const &permutation,
...
ParallelTensorDimDegrees
  permute_parallel_tensor_dim_degrees(TensorDimPermutation const &permutation,
  ...
  return ParallelTensorDimDegrees{
    ...
    /*shard_degrees=*/permute_ff_ordered(permutation, parallel_tensor_dim_degrees.shard_degrees),
  };
}

ParallelTensorDims 
  permute_parallel_tensor_dims(TensorDimPermutation const &permutation,
                               ParallelTensorDims const &parallel_tensor_dims) {
  return ParallelTensorDims{
    /*shard_dims=*/permute_ff_ordered(permutation, parallel_tensor_dims.shard_dims),
    /*replica_dims=*/parallel_tensor_dims.replica_dims,
  };
}

ParallelTensorShape
  permute_parallel_tensor_shape(TensorDimPermutation const &permutation,
                                ParallelTensorShape const &parallel_tensor_shape) {
  return ParallelTensorShape{
    /*dims=*/permute_parallel_tensor_dims(permutation, parallel_tensor_shape.dims),
...
::FlexFlow::TensorDimPermutation adl_serializer<::FlexFlow::TensorDimPermutation>::from_json(json const &j) {
...
Gen<::FlexFlow::TensorDimPermutation> Arbitrary<::FlexFlow::TensorDimPermutation>::arbitrary() {
  using namespace ::FlexFlow;

  Gen<std::vector<ff_dim_t>> key_permutation_gen = 
  ...
  return gen::construct<TensorDimPermutation>(
    gen::apply(
...
size_t hash<::FlexFlow::TensorDimPermutation>::operator()(::FlexFlow::TensorDimPermutation const &p) const {
