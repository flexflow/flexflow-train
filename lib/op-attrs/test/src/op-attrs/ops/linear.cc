  ...
  TEST_CASE("Linear shape inference") {
    ...
    auto make_input = [&](SumDegree o_sum,
      ...
      return lift_to_parallel_with_degrees(
          input, o_sum, o_eq, FFOrdered<nonnegative_int>{o_batch, o_extra_dim, o_channel});
    };
    ...
    auto make_output = [&](SumDegree o_sum,
                           ...
                           nonnegative_int o_channel) {
      return lift_to_parallel_with_degrees(
          output, o_sum, o_eq, FFOrdered<nonnegative_int>{o_batch, o_extra_dim, o_channel});
    };
    ...
    auto make_projection = [&](SumDegree o_sum,
      ...
      return lift_to_parallel_with_degrees(
          projection, o_sum, o_eq, FFOrdered<nonnegative_int>{o_inchannel, o_outchannel});
    };

    auto make_bias =
        [&](SumDegree o_sum, DiscardCopyDegree o_eq, nonnegative_int o_outchannel) {
          return lift_to_parallel_with_degrees(
    ...
    SUBCASE("reduction parallelism") {
      ...
      ParallelTensorShape par_input = make_input(
          SumDegree{input_sum_degree}, DiscardCopyDegree{1_n}, 1_n, 1_n, degree);
...

      {
        ...
        tl::expected<ParallelTensorShape, std::string> correct =
            make_projection(
                SumDegree{1_n}, DiscardCopyDegree{input_sum_degree}, degree, 1_n);
        CHECK(result == correct);
      }
    ...
    SUBCASE("output channel parallelism") {
      ...
      ParallelTensorShape par_input = make_input(
          SumDegree{input_sum_degree}, DiscardCopyDegree{degree}, 1_n, 1_n, 1_n);
...

      {
        ...
        tl::expected<ParallelTensorShape, std::string> correct =
            make_projection(
                SumDegree{1_n}, DiscardCopyDegree{input_sum_degree}, 1_n, degree);
        CHECK(result == correct);
      }
