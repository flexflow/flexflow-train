...
TEST_SUITE(FF_TEST_SUITE) {
  TEST_CASE("get_parallel_tensor_dim_ordering") {
    DimOrdering<parallel_tensor_dim_idx_t>
      ordering = get_parallel_tensor_dim_ordering();
    ...
    SUBCASE("discard is less than shard dim") {
      bool result = ordering.lt(discard_copy_dim_idx(), shard_dim_idx(ff_dim_t{0_n}));
      bool correct = true;
    ...
    SUBCASE("shard dim 0 is less than shard dim 1") {
      bool result = ordering.lt(shard_dim_idx(ff_dim_t{0_n}), shard_dim_idx(ff_dim_t{1_n}));
      bool correct = true;
