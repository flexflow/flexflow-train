...
bidict<parallel_tensor_guid_t, OutputGraphExprNodeOutput>
  ...
  for (auto const &[parallel_layer, output_graph_expr_node] : m.node_mapping) {
    ...
    std::unordered_map<TensorSlotName, OutputGraphExprNodeOutput> output_graph_expr_outputs =
        get_node_outputs(output_graph_expr, output_graph_expr_node);

    bidict<parallel_tensor_guid_t, OutputGraphExprNodeOutput>
        mapping_for_layer = bidict_from_pairs(
          values(zip_values_strict(layer_outputs, output_graph_expr_outputs)));
