...
static bidict<ParallelTensorSpaceCoordinate, MachineSpaceCoordinate>
  ...
  std::unordered_set<parallel_tensor_use_t> consumers = mpcg_get_parallel_tensor_uses(mpcg, output_tensor_guid);
  ...
  bidict<ParallelTensorSpaceCoordinate, MachineSpaceCoordinate> result =
    merge_disjoint_bidicts(
      transform(consumers,
                ...
                {
                  parallel_layer_guid_t consumer_layer = parallel_tensor_use_get_layer(use);
...
static DynamicNodeInvocation
  ...
  ManyToOne<TensorSlotName, parallel_tensor_guid_t> incoming = mpcg_get_incoming_tensors(mpcg, layer);
  ...
  bidict<TensorSlotName, parallel_tensor_guid_t> outgoing = mpcg_get_outgoing_tensors(mpcg, layer);
  TensorSlotName output_slot_name = TensorSlotName::OUTPUT;
  parallel_tensor_guid_t output_tensor_guid = require_only_key(outgoing.l_to_r(), output_slot_name);
...
DynamicOpenDataflowGraph make_dynamic_open_dataflow_graph_from_mapped_pcg(
  ...
  auto mk_invocation = [&](parallel_layer_guid_t layer, ParallelLayerAttrs const &attrs)
    -> DynamicNodeInvocation
  {
    if (attrs.op_attrs.is_parallel_replicate()) {
      ...
      auto mk_value_attrs = [&](parallel_tensor_guid_t const &tensor) -> DynamicValueAttrs
      {
      ...
      std::unordered_map<DynamicTensorSlot, DynamicValueAttrs> result_inputs =
          map_keys_and_values(get_incoming_tensors(pcg, layer),
                              mk_slot,
                              mk_value_attrs);

      std::unordered_map<DynamicTensorSlot, DynamicValueAttrs> result_outputs =
          map_keys_and_values(get_outgoing_tensors(pcg, layer),
                              mk_slot,
                              mk_value_attrs);
  ...
  return dynamic_open_dataflow_graph_from_invocation_set(
    transform_pairs(
      unordered_set_of(get_parallel_layer_attrs_mapping(pcg)),
      mk_invocation));
}
