...
void RuntimeTaskBinding::bind(TensorSlotName name, symbolic_forward_tensor_guid_t const &binding) {
  this->tensor_bindings.insert({
    training_tensor_slot_id_t{name, TrainingTensorType::FORWARD},
    symbolic_training_tensor_guid_t{binding},
  });
}

void RuntimeTaskBinding::bind_grad(TensorSlotName name,
...
void RuntimeTaskBinding::bind_optimizer(TensorSlotName name,
...
void RuntimeTaskBinding::bind_loss(TensorSlotName name, symbolic_loss_tensor_guid_t const &binding) {
  this->tensor_bindings.insert({
    training_tensor_slot_id_t{name, TrainingTensorType::LOSS},
    symbolic_training_tensor_guid_t{binding},
  });
}

void RuntimeTaskBinding::insert_arg_spec(arg_slot_id_t name, RuntimeArgSpec const &arg_spec) {
...
std::tuple<
    std::unordered_map<training_tensor_slot_id_t, symbolic_training_tensor_guid_t> const &,
    std::unordered_map<arg_slot_id_t, RuntimeArgSpec> const &>
