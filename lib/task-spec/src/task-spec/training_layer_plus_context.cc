...
std::vector<TrainingTensorGroupWithAttrs> get_training_tensor_groups_with_attrs_for_role(
...
TrainingTensorGroupWithAttrs get_training_tensor_group_with_attrs_for_role_and_index(
  TrainingLayerPlusContext const &training_layer_plus_context,
  TensorRole tensor_role,
  nonnegative_int index) {

  return get_training_tensor_groups_with_attrs_for_role(
    training_layer_plus_context,
    tensor_role).at(index.unwrap_nonnegative());
}
...
std::vector<TensorShape> get_weight_tensor_shapes(TrainingLayerPlusContext const &l) {
  return transform(l.weight_tensor_groups,
                   [](TrainingTensorGroupWithAttrs const &g) {
                     return g.tensor_attrs.shape;
                   });
...
std::vector<TensorShape> get_output_tensor_shapes(TrainingLayerPlusContext const &l) {
  return transform(l.output_tensor_groups,
                   [](TrainingTensorGroupWithAttrs const &g) {
                     return g.tensor_attrs.shape;
                   });
...
TrainingLayerTensorGroupSignature get_tensor_group_signature(TrainingLayerPlusContext const &l) {
  return TrainingLayerTensorGroupSignature{
    /*input_tensor_groups=*/transform(l.input_tensor_groups, tensor_group_without_attrs),
    /*weight_tensor_groups=*/transform(l.weight_tensor_groups, tensor_group_without_attrs),
    /*output_tensor_groups=*/transform(l.output_tensor_groups, tensor_group_without_attrs),
  };
}

CGOperatorTensorShapeSignature get_cg_op_shape_signature(TrainingLayerPlusContext const &l) {
