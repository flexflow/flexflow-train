...
TEST_SUITE(FF_TEST_SUITE) {
  TEST_CASE("perform_machine_slicing_for_invocation") {
    ...
    DynamicValueAttrs graph_input1 = mk_value(0, TensorSlotName::OUTPUT, std::nullopt);
    DynamicValueAttrs graph_input2 = mk_value(1, TensorSlotName::OUTPUT, std::nullopt);
    DynamicValueAttrs invocation1_output1 = mk_value(invocation1_id, TensorSlotName::OUTPUT_1, std::nullopt);
    DynamicValueAttrs invocation1_output2 = mk_value(invocation1_id, TensorSlotName::OUTPUT_2, std::nullopt);
    DynamicValueAttrs invocation2_output1 = mk_value(invocation2_id, TensorSlotName::OUTPUT_4, std::nullopt);
    DynamicValueAttrs invocation3_output1 = mk_value(invocation3_id, TensorSlotName::OUTPUT_1, std::nullopt);
    ...
    SUBCASE("task does not exist on MachineCoord") {
      DynamicOpenDataflowGraph result = 
        perform_machine_slicing(unsliced, mc3);

      DynamicOpenDataflowGraph correct = 
        dynamic_open_dataflow_graph_from_invocation_set(std::unordered_set<DynamicNodeInvocation>{});
