    ...
    std::unordered_set<std::pair<C, R>> const &rhs)
{
  ...
  std::unordered_set<L> lhs_ls = transform(lhs, [](std::pair<L, C> const &lc) { return lc.first; });
  std::unordered_set<C> lhs_cs = transform(lhs, [](std::pair<L, C> const &lc) { return lc.second; });
  std::unordered_set<C> rhs_cs = transform(rhs, [](std::pair<C, R> const &cr) { return cr.first; });
  std::unordered_set<R> rhs_rs = transform(rhs, [](std::pair<C, R> const &cr) { return cr.second; });
