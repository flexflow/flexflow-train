...
std::vector<std::tuple<A, B, C>> zip3_strict(std::vector<A> const &as,
  ...
  if (!(as.size() == bs.size() && bs.size() == cs.size())) {
    throw mk_runtime_error(fmt::format("zip3_strict requires as, bs, and cs to have the same length, but received as={} (length {}), bs={} (length {}), and cs={} (length {})", as, as.size(), bs, bs.size(), cs, cs.size()));
  }
