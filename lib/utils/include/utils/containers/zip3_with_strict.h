...
template <typename F, typename A, typename B, typename C, typename Result = std::invoke_result_t<F, A, B, C>>
std::vector<Result> zip3_with_strict(std::vector<A> const &v_a,
  ...
  ASSERT(v_a.size() == v_b.size() && v_b.size() == v_c.size(),
         "zip3_with_strict requires inputs to have the same length, but received mismatched lengths", 
         v_a.size(), v_b.size(), v_c.size());
