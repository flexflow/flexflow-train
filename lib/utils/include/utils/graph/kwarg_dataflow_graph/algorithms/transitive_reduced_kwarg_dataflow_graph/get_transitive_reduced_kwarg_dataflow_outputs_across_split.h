...
std::unordered_set<KwargDataflowOutput<SlotName>> get_transitive_reduced_kwarg_dataflow_outputs_across_split(
   TransitiveReducedKwargDataflowGraphView<SlotName> const &tr_g, BinarySeriesSplit const &split) {

  return transform(get_transitive_reduced_kwarg_dataflow_edges_across_split(tr_g, split),
