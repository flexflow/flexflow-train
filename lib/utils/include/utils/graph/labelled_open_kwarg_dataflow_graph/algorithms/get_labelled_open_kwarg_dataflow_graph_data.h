...
{
  return LabelledOpenKwargDataflowGraphData<NodeLabel, ValueLabel, GraphInputName, SlotName>{
    /*nodes=*/generate_map(get_nodes(g),
                           [&](Node const &n) -> NodeLabel {
                             return g.at(n); 
                           }),
    /*edges=*/get_all_open_kwarg_dataflow_edges(g),
    /*inputs=*/get_all_kwarg_dataflow_graph_inputs(g),
    /*outputs=*/generate_map(get_all_open_kwarg_dataflow_values(g),
                             [&](OpenKwargDataflowValue<GraphInputName, SlotName> const &v) -> ValueLabel {
                               return g.at(v); 
                             }),
  };
}
