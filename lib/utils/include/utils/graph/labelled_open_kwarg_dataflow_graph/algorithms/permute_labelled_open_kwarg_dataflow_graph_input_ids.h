    ...
    bidict<KwargDataflowGraphInput<GraphInputName>, KwargDataflowGraphInput<GraphInputName>> const &new_input_to_old_input)
{
  ...
  OpenKwargDataflowGraphView<GraphInputName, SlotName> permuted = permute_open_kwarg_dataflow_graph_input_ids(
      static_cast<OpenKwargDataflowGraphView<GraphInputName, SlotName>>(g), new_input_to_old_input);

  auto old_input_from_new = [&](KwargDataflowGraphInput<GraphInputName> const &i) 
  ...
  auto old_value_from_new = [&](OpenKwargDataflowValue<GraphInputName, SlotName> const &new_value) {
    return new_value.template visit<OpenKwargDataflowValue<GraphInputName, SlotName>>(overload{
        ...
        [&](KwargDataflowGraphInput<GraphInputName> const &i) { 
          return OpenKwargDataflowValue<GraphInputName, SlotName>{old_input_from_new(i)};
        },
    });
  };
  ...
  std::unordered_map<OpenKwargDataflowValue<GraphInputName, SlotName>, ValueLabel> value_labels =
      generate_map(get_all_open_kwarg_dataflow_values(permuted),
                   [&](OpenKwargDataflowValue<GraphInputName, SlotName> const &new_value) {
                     return g.at(old_value_from_new(new_value));
                   });

  return open_kwarg_dataflow_graph_view_with_labelling(permuted, node_labels, value_labels);
}
