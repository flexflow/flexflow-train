#ifndef _FLEXFLOW_LIB_UTILS_INCLUDE_UTILS_GRAPH_LABELLED_OPEN_KWARG_DATAFLOW_GRAPH_ALGORITHMS_REWRITE_LABELS_H
#define _FLEXFLOW_LIB_UTILS_INCLUDE_UTILS_GRAPH_LABELLED_OPEN_KWARG_DATAFLOW_GRAPH_ALGORITHMS_REWRITE_LABELS_H
...
template <
    ...
    typename NewValueLabel =
        std::invoke_result_t<F, OpenKwargDataflowValue<GraphInputName, SlotName> const &, ValueLabel const &>>
LabelledOpenKwargDataflowGraphView<NewNodeLabel, NewValueLabel, GraphInputName, SlotName> rewrite_labels(
  ...
  auto get_new_value_label = [&](OpenKwargDataflowValue<GraphInputName, SlotName> const &v) -> NewValueLabel {
    return f(v, g.at(v));
  };

  std::unordered_map<Node, NewNodeLabel> node_labels =
      generate_map(get_nodes(g), get_new_node_label);
  std::unordered_map<OpenKwargDataflowValue<GraphInputName, SlotName>, NewValueLabel> value_labels =
      generate_map(get_all_open_kwarg_dataflow_values(g), get_new_value_label);
