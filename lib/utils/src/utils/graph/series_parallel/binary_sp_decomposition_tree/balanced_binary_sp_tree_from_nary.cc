...
BinarySPDecompositionTree balanced_binary_sp_tree_from_nary(
  ...
  std::function<BinarySPDecompositionTree(ParallelSplit const& p)> from_parallel;
  ...
  from_parallel = [&](ParallelSplit const &s) -> BinarySPDecompositionTree {
    ...
    if (children.size() == 1) {
      ...
      return BinarySPDecompositionTree{BinaryParallelSplit{from_parallel_child(children[0]), from_parallel_child(children[1])}};
    }
    auto s1 = unordered_multiset_of(subvec(children, std::nullopt, children.size()/2));
    auto s2 = unordered_multiset_of(subvec(children, children.size()/2, std::nullopt));
    return BinarySPDecompositionTree{BinaryParallelSplit{from_parallel(ParallelSplit{s1}), from_parallel(ParallelSplit{s2})}};
    };
  ...
  if (children.size() == 1) {
    ...
    return BinarySPDecompositionTree{BinarySeriesSplit{from_series_child(children[0]), from_series_child(children[1])}};
  }
  auto s1 = subvec(children, std::nullopt, children.size()/2);
  auto s2 = subvec(children, children.size()/2, std::nullopt);
  return BinarySPDecompositionTree{BinarySeriesSplit{from_series(SeriesSplit{s1}), from_series(SeriesSplit{s2})}};
  };
