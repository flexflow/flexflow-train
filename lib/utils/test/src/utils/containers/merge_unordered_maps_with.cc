  ...
  TEST_CASE("merge_unordered_maps_with") {
    ...
    RC_SUBCASE(
        ...
        [&](std::unordered_map<int, std::string> const &lhs,
          ...
          std::unordered_map<int, std::string> from_binary_merge_unordered_maps_with =
              binary_merge_unordered_maps_with(lhs, rhs, string_concat);

          CHECK(from_merge_unordered_maps_with == from_binary_merge_unordered_maps_with);
        });
    ...
    SUBCASE("maps overlap") {
      ...
      std::unordered_map<int, std::string> result =
          merge_unordered_maps_with(std::vector{map1, map2, map3}, string_concat);
    ...
    SUBCASE("maps do not overlap") {
      ...
      std::unordered_map<int, std::string> result =
          merge_unordered_maps_with(std::vector{map1, map2, map3}, fail_if_called);
