  ...
  TEST_CASE("permute_with_key") {
    SUBCASE("generates all permutations within period") {
      ...
      std::unordered_set<std::vector<std::string>> 
        generated_permutations = unordered_set_of(transform(range(max_permutations),
                                                            [&](int key) {
                                                              return permute_with_key(key, input);
                                                            }));
      std::unordered_set<std::vector<std::string>>
        all_permutations = unordered_set_of(get_all_permutations(input));

      CHECK(generated_permutations == all_permutations);
    }

    RC_SUBCASE("generates valid permutations", 
               [](int key, std::vector<int> input) {
                 std::vector<int> permuted = permute_with_key(key, input);
                 return std::is_permutation(input.cbegin(), input.cend(), 
                                            permuted.cbegin(), permuted.cend());
               });
  }
}
