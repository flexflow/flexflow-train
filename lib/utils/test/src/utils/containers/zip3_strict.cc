...
TEST_SUITE(FF_TEST_SUITE) {
  TEST_CASE("zip3_strict(std::vector<A>, std::vector<B>, std::vector<C>)") {
    SUBCASE("types are same") {
      ...
      std::vector<std::tuple<int, int, int>> result = zip3_strict(input_a, input_b, input_c);
    ...
    SUBCASE("types are different") {
      ...
      std::vector<std::tuple<int, std::string, std::vector<int>>> result = zip3_strict(input_a, input_b, input_c);
